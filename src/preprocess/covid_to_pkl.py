...
class DataSet(object):
	...
	def parse_data(self, fpath,  with_label=True):
		...
		# Y is the shape of (len(XD),len(XT)) 
		XD = []
		XT = []
...
def save_interactions_to_csv(label_row_inds, label_col_inds, chembl_id_list, smiles_list, XD,
	...
	# We don't have the CID for each CHEMBL, so print instead the CHEMBL ID
	#chembl_to_cid = load_cid_chembl_lookup(base_path)

	reference_dic = {}

	with open("%s/covid/covid_kd.csv" % base_path, "w") as f:
	...
	with open("%s/covid/reference_dic.cpkl" % base_path, "wb") as handle:
...
def get_split(base_path, problem_type, max_seq_len, max_smi_len):
	...
	tst_set = [i for i in range(len(XD))]
	outer_train_sets = []
	...
	# cPickle.dump((XD, XT, Y, trn_sets, dev_sets, tst_set, row_idx, col_idx), open('%s/covid/covid.cpkl' % base_path, 'wb'))
